...
```C++
long long GetMaxSubSum(long long* a,long long length){
	long long dp = 0,ans = INT_MIN;
...
```C++
long long GetFastPower(long long a,long long b){
	long long res = 1;
	while(b > 0){
		if(b & 1) res = res * a; 
		a = a * a;
...
```C++
long long GetFastPowerMod(long long a,long long b,long long p){
	long long res = 1;
	while(b > 0){
		if(b & 1) res = (res * a) % p; 
		a = (a * a) % p;
